...
const SignUp = () => {
  const [uiImage, setUiImage] = useState(boy);
  ...
  const onHandleSubmit = handleSubmit(async (data) => {
    const { userName, email, location, password } = data;
    ...
    const url = "http://localhost:1372/api/user/register";
    ...
    <Container>
      <Wrapper>
        <Card onSubmit={onHandleSubmit}>
          ...
          <ImageInput type="file" onChange={uploadImage} id="boy" />
          ...
          <Holder>
            ...
            <Hold>
              <DivError>{errors.location && errors?.location.message}</DivError>
